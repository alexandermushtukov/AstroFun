...
!============================================================================================
! OUTPUT GRID (LAYER-LOWER-BOUNDARY VERSION):
!
! tau_targets(1)=0
! tau_targets(2..n)=logspace(tau_min..tau_max)
!
! Define layer i as [tau_targets(i), tau_targets(i+1)] for i=1..n-1.
! We output values at LOWER boundary of layer i:
!   tau_lower(i) = tau_targets(i+1) for i=1..n-1
!   tau_lower(n) = tau_targets(n)   (duplicate last boundary to keep n rows)
!
...
!
! Requires tau_min > 0 when n>2.
!============================================================================================
subroutine acc_atm_structure_5(mas_x_rho_tau_2,n,tau_min,tau_max,x_scale, &
                              g14,mas_tau_TkeV,n_mas,dot_m_6,ln_Lambda,rho0,F_tau)
implicit none
integer, intent(in)  :: n, n_mas
real*8,  intent(in)  :: tau_min, tau_max, g14, dot_m_6, ln_Lambda, rho0
...
real*8  :: eps24, eps_old
real*8  :: rho_floor, beta_floor, T_floor
real*8  :: tau_targets(n), tau_lower(n), log_ratio, expo
...
external :: find_H_ordered_inc
  ...
  !---- lower boundaries for each output row
  if (n .eq. 1) then
    tau_lower(1) = tau_targets(1)   ! 0
  else
    do i = 1, n-1
      tau_lower(i) = tau_targets(i+1)
    end do
    tau_lower(n) = tau_targets(n)
  end if

  !---- We now WRITE rows at tau = tau_lower(i), so first target is tau_lower(1)
  i = 1
...
return
end subroutine acc_atm_structure_5
!============================================================================================
